...
static func serialize_state(incoming_state: StateData, incoming_node: Node3D) -> void:
	if incoming_node.has_method(GroupData.SET_ASSET_STATE):
		...
		incoming_node.call(GroupData.SET_ASSET_STATE, new_state)
	else:
		var missing_log: String = "%s missing from incoming node" % GroupData.SET_ASSET_STATE
