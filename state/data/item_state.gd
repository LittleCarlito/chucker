...
signal state_item_change(new_state: ItemState)
...
func _init(incoming_name: String, incoming_windows: Dictionary = {}) -> void:
	self._owner_name = incoming_name
	self._state_windows = incoming_windows
	_validate_window_configuration()

func reset_state() -> StateConstants.STATE:
	var lowest_state: StateConstants.STATE = StateConstants.STATE.READY
	...
	if found_state:
		...
		_current_state = StateConstants.STATE.READY
...
func duplicate() -> ItemState:
	var new_state := ItemState.new(_owner_name, _state_windows.duplicate(true))
...
func get_current_state() -> StateConstants.STATE:
	return self._current_state

func get_nearest_state(incoming_value: float) -> StateConstants.STATE:
	if _state_values.is_empty():
		Logger.warn(_CLOSEST_STATE_NOT_FOUND, [_owner_name, incoming_value], self)
		return StateConstants.STATE.READY
	var closest_state: StateConstants.STATE = StateConstants.STATE.READY
...
func can_transition(to_state: StateConstants.STATE) -> bool:
	if self._valid_transitions.has(self._current_state):
		var current_valid_transitions: Array[StateConstants.STATE] = _get_sorted_transitions(_current_state)
...
func try_set_state(to_state: StateConstants.STATE) -> bool:
...
func get_all_states() -> Array[StateConstants.STATE]:
	var all_states: Array[StateConstants.STATE] = []
	for state in _valid_transitions.keys():
...
func peak_next_state() -> StateConstants.STATE:
...
func peak_next_state_value() -> float:
	...
	if _state_values.has(next_state):
		...
		var next_state_string: String = StateConstants.get_state_string(next_state)
		Logger.warn(self._NO_VALUE, [self._owner_name, next_state_string], self)
		return 0

func next_states() -> Array[StateConstants.STATE]:
	return _collect_states(true)

func transition_next_state() -> StateConstants.STATE:
	var next_state := _find_next_valid_state()
	if next_state != _current_state:
		_current_state = next_state
		state_item_change.emit(self)
	else:
		Logger.debug(self._MAX_VALID_STATE, [_owner_name, _current_state], self)
	return _current_state

func peak_previous_state() -> StateConstants.STATE:
...
func previous_states() -> Array[StateConstants.STATE]:
	return _collect_states(false)

func transition_previous_state() -> StateConstants.STATE:
	var prev_state := _find_previous_valid_state()
	if prev_state != _current_state:
...
func get_state_value(incoming_value: StateConstants.STATE) -> float:
	if self._state_values.has(incoming_value):
		return self._state_values[incoming_value]
	else:
		var incoming_state_string: String = StateConstants.get_state_string(incoming_value)
		Logger.warn(self._NO_VALUE, [self._owner_name, incoming_state_string], self)
		return 0

func is_valid_state(incoming_state: StateConstants.STATE) -> bool:
...
func to_string() -> String:
	var current_state_name: String = StateConstants.get_state_string(_current_state)
	var details: String = "ItemState[%s]: current_state=%s(%d), duration=%.2f" % [_owner_name, current_state_name, _current_state, _current_state_duration]
	details += ", windows=%d, transitions=%d, values=%d" % [_state_windows.size(), _valid_transitions.size(), _state_values.size()]
	return details

func _get_sorted_transitions(state: StateConstants.STATE) -> Array[StateConstants.STATE]:
	if _valid_transitions.has(state):
		var transitions: Array[StateConstants.STATE] = _valid_transitions[state]
		transitions.sort()
		return transitions
	else:
...
func _collect_states(forward: bool) -> Array[StateConstants.STATE]:
	var transitions := _get_sorted_transitions(_current_state)
	var result: Array[StateConstants.STATE] = []
...
func _find_next_valid_state() -> StateConstants.STATE:
	var transitions := _get_sorted_transitions(_current_state)
	for s in transitions:
		if s > _current_state:
			return s
...
func _find_previous_valid_state() -> StateConstants.STATE:
	var transitions := _get_sorted_transitions(_current_state)
	for s in transitions:
		if s < _current_state:
			return s
...
func _validate_window_configuration() -> bool:
	...
	for state in sorted_states:
		...
		if current_value < previous_value:
			var state_string: String = StateConstants.get_state_string(state)
