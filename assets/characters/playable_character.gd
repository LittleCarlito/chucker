...
func _handle_player_action(_delta: float) -> void:
	Logger.error(_UNIMPLEMENTED_LOG, [_HANDLE_PLAYER_ACTION], self)

## Handles camera/aiming related actions
func _handle_camera_controls() -> void:
	Logger.error(_UNIMPLEMENTED_LOG, [_HANDLE_CAMERA_CONTROLS], self)

## Handle player pressing interact button
func _handle_player_interact() -> void:
	Logger.error(_UNIMPLEMENTED_LOG, [_HANDLE_PLAYER_INTERACT], self)

## Detects and executes movements
func _handle_movement(_delta: float) -> void:
	Logger.error(_UNIMPLEMENTED_LOG, [_HANDLE_MOVEMENT], self)

## Unequips the currently equipped item
func unequip_item() -> void:
	Logger.error(_UNIMPLEMENTED_LOG, [_UNEQUIP_ITEM], self)

## Returns the height of Chuck
func get_height() -> float:
	Logger.error(_UNIMPLEMENTED_LOG, [_GET_HEIGHT], self)
	return 0
