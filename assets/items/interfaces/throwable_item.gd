	...
	if Input.get_mouse_mode() == Input.MOUSE_MODE_CAPTURED and not justLaunched:
		if ownerVar != null and event is InputEventMouseMotion:
			...
			if GLOBAL_SETTINGS.CONTROLS.get(CONSTANTS.INVERT_VERTICAL, GLOBAL_SETTINGS.CONTROLS_DEFAULTS.INVERT_VERTICAL):
				vInversionValue = 1
			ownerVar.rotation.y -= hInversionValue * (event.relative.x / 1000 * GLOBAL_SETTINGS.CONTROLS.get(CONSTANTS.HORIZONTAL_SENSITIVITY, GLOBAL_SETTINGS.CONTROLS_DEFAULTS.HORIZONTAL_SENSITIVITY))
			var rotationAmount = vInversionValue * (GLOBAL_SETTINGS.CONTROLS.get(CONSTANTS.VERTICAL_SENSITIVITY, GLOBAL_SETTINGS.CONTROLS.VERTICAL_SENSITIVITY) * (event.relative.y / 1000 * GLOBAL_SETTINGS.CONTROLS.get(CONSTANTS.VERTICAL_SENSITIVITY, GLOBAL_SETTINGS.CONTROLS_DEFAULTS.VERTICAL_SENSITIVITY)))
