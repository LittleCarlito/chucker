...
func draw_aim_line(multiplier: float, x_offset: float = 0) -> Array[Vector3]:
	...
	# Handle aimNode
	aim_node.position = global_position
	aim_node.basis = global_basis
	...
	launch_control_node.position = global_position
	launch_control_node.basis = global_basis
	...
	aim_control_node.position = global_position
	...
	# Draw the curve
	return DrawUtil.curve(global_position, launch_control_node.position, aim_control_node.position, aim_node.position)
