# TODO See if this and charge disk really need to be of the same inheritance type; If they do make their scene structure similar to others
...
static func new_disk(new_thrower: ChuckChucker, new_camera: Camera3D, new_type: CONSTANTS.DISK_TYPE) -> PullDisk:
	var new_disk: PullDisk = disk_scene.instantiate()
	new_disk.prepare_item(new_type, new_thrower, new_camera)
	return new_disk
