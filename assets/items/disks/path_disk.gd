...
func _process(delta: float) -> void:
	if is_instance_valid(path_follow):
		disk_mesh.rotation.y += (flight_data.get_flight_spin() * delta) * GameConfig.DEFAULTS.spin_multiplier
...
func _physics_process(delta: float) -> void:
	if _launched:
		...
		if GameConfig.DEFAULTS.flight_detail and not _details_logged:
			flight_data.print_details()
			_details_logged = true
		# Move disk in space
		if path_follow.progress_ratio < 1:
			_apply_roll_intensity()
...
func _apply_roll_intensity() -> void:
	if flight_data.get_flight_power() > GameConfig.DEFAULTS.min_pull_for_offset:
		...
		if GameConfig.DEFAULTS.flight_detail:
			Log.debug("Roll intensity at percentage %03f is %03f", [path_follow.progress_ratio, current_roll_intensity], self)
		var roll_modifier: float = current_roll_intensity * 20
		disk_mesh.rotation.z = roll_modifier
	else:
		if GameConfig.DEFAULTS.flight_detail:
